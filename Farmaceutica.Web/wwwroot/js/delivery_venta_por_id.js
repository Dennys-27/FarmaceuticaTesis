...
$(document).ready(function () {
    ...
    tablaUsuarios = $("#table_data").DataTable({
        ...
        ajax: {
            url: "/Delivery/ListarVentasParaAsignarDeliveryPorId",
            type: "POST",
            data: function (d) {
...
$("#mantenimiento_form").on("submit", function (e) {
    ...
    let data = {
        ventaId: $("#venta_id").val(),                  // ID de la venta
        estadoDelivery: $("#select_estado").val()      // ID del estado seleccionado (3,4,5)
    };

    // Validación básica
    if (!data.estadoDelivery) {
        alert("Seleccione un estado de delivery");
        return;
    }

    // Enviar datos al servidor
    $.post("/Delivery/ProcesarDeliveryPorId", data, function (resp) {
