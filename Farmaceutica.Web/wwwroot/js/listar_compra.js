...
function editar(numeroFactura) {
    $.post("/Compra/Mostrar", { numeroFactura: numeroFactura }, function (response) {
        if (!response.success) {
        ...
        $("#detalleCompraTable").DataTable({
            ...
            responsive: true,
            language: {
                sEmptyTable: "No hay productos para esta compra",
            },
        });

        $("#modalmantenimiento").modal("show");
    }).fail(() => {
        Swal.fire("Error", "No se pudo obtener los datos de la compra.", "error");
    });
}
