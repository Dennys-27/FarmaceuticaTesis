...
namespace Farmaceutica.Web.Controllers.Mantenimentos
{
    public class ClientesController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> ComboClientes()
        {
            var clientes = await _clienteRepository.GetComboAsync();
            return Json(clientes);
        }

        [HttpPost]
        public async Task<JsonResult> BuscarCliente([FromBody] BuscarClienteRequest request)
        {
            if (request == null || string.IsNullOrEmpty(request.Usuario))
                return Json(new { success = false, message = "Datos no válidos" });

            var cliente = await _clienteRepository.BuscarClientePorUsuarioAsync(request.Usuario);
